...
{ 
  // map from blockIdx to pixel position in image
	...
	if (globalThreadIndexx < imagesizex && globalThreadIndexy < imagesizey) // If inside image
	{
		...
		for(int dy=-kernelSizeY;dy<=kernelSizeY;dy++)
			...
			{
				//pixel index in shared memory
				int yy = shared_y + dy;
				int xx = shared_x + dx;
				
				//accumulate kernel values
				sumR += sharedMemR[(yy)*sharedW+(xx)];
				sumG += sharedMemG[(yy)*sharedW+(xx)];
				sumB += sharedMemB[(yy)*sharedW+(xx)];
			}
			__syncthreads();
...
void computeImages(int kernelsizex, int kernelsizey)
{
	...
	// Launch the kernel
	filter<<<dimGrid,dimBlock>>>(dev_input, dev_bitmap, imagesizex, imagesizey);
